    ...
    <div className={`relative w-full ${className}`}>
      <div
        ...
        className={`max-h-[600px] overflow-y-auto p-4 ${
          displayScrollbar
            ? '[&::-webkit-scrollbar]:w-[6px] [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-white/10 [&::-webkit-scrollbar-thumb]:rounded-[4px]'
            : 'scrollbar-hide'
        }`}
        ...
        {items.map((item, index) => (
          ...
          >
            {renderItem ? (
              ...
              <div
                className={`p-4 rounded-lg transition-colors ${itemClassName}`}
                style={{
                  backgroundColor: selectedIndex === index ? 'var(--color-surface-3)' : 'var(--color-surface-2)',
                  border: '1px solid rgba(255,255,255,0.06)',
                }}
              >
                <p style={{ color: 'var(--color-text-primary)' }} className="m-0">{item}</p>
              </div>
            )}
          </AnimatedItem>
        ))}
